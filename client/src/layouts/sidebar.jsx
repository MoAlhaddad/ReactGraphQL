import { forwardRef, useEffect, useState } from "react";
import { NavLink } from "react-router-dom";
import { useUser } from "@clerk/clerk-react"; // Assuming Clerk for auth

import logo from "../assets/choosescribe_logo.jpeg";
import { cn } from "../utils/cn";
import PropTypes from "prop-types";

// Admin nav items
const ADMIN_NAV_ITEMS = [
  {
    title: "Employees",
    links: [
      { label: "Add Employee", icon: UserPlus, path: "/new-employee" },
    ]
    
  },
  {  title: "Schedule",
    links: [
    { label: "Set Up Schedule", icon: ClipboardCheck, path: "/set-up-schedule" },

    ]},
     {  title: "Schedule",
    links: [
    { label: "This Weeks Schedule", icon: ClipboardCheck, path: "/schedule" },
    
    ]}
];

// Employee nav items
const EMPLOYEE_NAV_ITEMS = [
  {
    title: "Schedule",
    links: [
      { label: "My Schedule", icon: ClipboardCheck, path: "/my-schedule" },
    ],
  },
];
import { ClipboardCheck, UserPlus } from "lucide-react";

export const Sidebar = forwardRef(({ collapsed }, ref) => {
  const { user } = useUser();
  const [navItems, setNavItems] = useState([]);

  useEffect(() => {
    const isAdmin = user?.primaryEmailAddress?.emailAddress === "mohamadalhaddad25@gmail.com";
    setNavItems(isAdmin ? ADMIN_NAV_ITEMS : EMPLOYEE_NAV_ITEMS);
  }, [user]);

  return (
    <aside
      ref={ref}
      className={cn(
        "fixed z-[100] flex h-full w-[240px] flex-col overflow-x-hidden border-r border-slate-300 bg-white [transition:_width_300ms_cubic-bezier(0.4,_0,_0.2,_1),_left_300ms_cubic-bezier(0.4,_0,_0.2,_1),_background-color_150ms_cubic-bezier(0.4,_0,_0.2,_1),_border_150ms_cubic-bezier(0.4,_0,_0.2,_1)] dark:border-slate-700 dark:bg-slate-900",
        collapsed ? "md:w-[70px] md:items-center" : "md:w-[240px]",
        collapsed ? "max-md:-left-full" : "max-md:left-0"
      )}
    >
      <div className="flex gap-x-3 p-3">
        <NavLink to="/" className="inline-block">
        <img src={logo} alt="choosescribe" className="dark:hidden" />
        </NavLink>
      </div>
      <div className="flex w-full flex-col gap-y-4 overflow-y-auto overflow-x-hidden p-3 [scrollbar-width:_thin]">
        {navItems.map((NAVITEM) => (
          <nav
            key={NAVITEM.title}
            className={cn("sidebar-group", collapsed && "md:items-center")}
          >
            <p className={cn("sidebar-group-title", collapsed && "md:w-[45px]")}>{NAVITEM.title}</p>
            {NAVITEM.links.map((link) => (
              <NavLink
                key={link.label}
                to={link.path}
                className={cn("sidebar-item", collapsed && "md:w-[45px]")}
              >
                <link.icon size={22} className="flex-shrink-0" />
                {!collapsed && <p className="whitespace-nowrap">{link.label}</p>}
              </NavLink>
            ))}
          </nav>
        ))}
      </div>
    </aside>
  );
});

Sidebar.displayName = "Sidebar";

Sidebar.propTypes = {
  collapsed: PropTypes.bool,
};
